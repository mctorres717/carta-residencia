...
export default function GeneradorCartas() {
  ...
  const handlePrint = () => {
    ...
    const nombreSplit = propietarioSeleccionado.PROPIETARIO.split(' ');
    const nombre = (nombreSplit[0] || '') + ' ' + (nombreSplit[1] || '');
    const fechaDescarga = new Date().toLocaleDateString('es-VE').replace(/\//g, '-');
    document.title = `Carta de Residencia – ${nombre} – Apto ${propietarioSeleccionado.APARTAMENTO} – ${fechaDescarga}`;
    window.print();
  };
  ...
  return (
    <div className="min-h-screen bg-black text-white font-serif antialiased pb-12 print:bg-white print:text-black print:p-0">
      <style>{`@media print { @page { size: A4 portrait; margin: 2cm; } .no-print { display: none !important; } .print-area { display: block !important; height: 100vh !important; } }`}</style>
      ...
      <div className="print-area max-w-[800px] mx-auto bg-white text-black mt-8 p-0 shadow-2xl min-h-[90vh] flex flex-col justify-between font-serif text-[12pt]">
        {propietarioSeleccionado ? (
          <div className="p-[2.5cm]">
            <div className="flex justify-between items-center border-b-2 border-black pb-4 mb-8">
              <img src="/ministerio.png" alt="Min" className="h-16 w-auto" />
              <img src="/logo_edificio.png" alt="Torre" className="h-16 w-auto" />
            </div>
            <h2 className="text-center text-[18pt] font-bold underline uppercase tracking-wide mb-10">Constancia de Residencia</h2>
            <p className="mb-6 leading-[1.8] font-normal">Quienes suscriben, en representación del <strong className="font-bold">COMITÉ MULTIFAMILIAR DE GESTIÓN (C.M.G.) DE LA TORRE D-10</strong>, hacen constar que el (la) ciudadano (a) <strong className="font-bold uppercase">{propietarioSeleccionado.PROPIETARIO}</strong>, titular de la cédula de identidad N° <strong className="font-bold">V-{propietarioSeleccionado.CEDULA}</strong>, habita en la: <strong className="font-bold">Torre D-10, Piso {propietarioSeleccionado.PISO}, Apartamento {propietarioSeleccionado.APARTAMENTO}</strong>, desde el mes de {propietarioSeleccionado["INICIO MES"]} del año {propietarioSeleccionado["INICIO AÑO"]}.</p>
            <p className="mb-6 leading-[1.8]">Tiempo durante el cual, el ciudadano ha mantenido una conducta ejemplar, observando los principios de convivencia ciudadana y respeto a las normas comunitarias establecidas en la edificación.</p>
            <p className="mb-6 leading-[1.8]">Constancia que se expide a petición de la parte interesada en la ciudad de Caracas, a los {fechaActual.diaLetras} ({fechaActual.diaNumero}) días del mes de {fechaActual.mesLetras} del año {fechaActual.anoNumero}.</p>
            ...
            <div className="mb-10 font-normal grid grid-cols-2 gap-x-12 text-center text-[11pt]">
              <div><p className="border-t border-black pt-2"><strong className="font-bold">Vocera Principal</strong></p><p><strong className="font-bold">Sindy Chacón</strong></p><p>C.I V- 17.693.292</p><p>Telf. (0424) 560-15-62</p></div>
              <div><p className="border-t border-black pt-2"><strong className="font-bold">Vocero Principal</strong></p><p><strong className="font-bold">Marcos Díaz</strong></p><p>C.I V- 16.662.440</p><p>Telf. (0414) 017-40-62</p></div>
            </div>
          ...
          <div className="h-full flex items-center justify-center text-neutral-400 no-print">Seleccione un propietario para generar la carta.</div>
        )}
      </div>
    </div>
  );
