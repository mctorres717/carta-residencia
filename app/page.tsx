...
// --- CONEXIÓN DIRECTA A SUPABASE (USANDO TUS CREDENCIALES) ---
...
export default function SistemaTorreD10() {
  ...
  // Estado para el Formulario de Actualización / Edición
  const [editingProp, setEditingProp] = useState<any>(null);

  // Carga inicial y tiempo real con Supabase
  useEffect(() => {
    const hoy = new Date();
    actualizarFecha(hoy);
    ...
    // Canal en tiempo real: Si actualizas en la pestaña, impacta al instante
  ...
  const fetchPropietarios = async () => {
    const { data, error } = await supabase
      ...
      .select('*');
    if (data) {
      // Ordenar naturalmente por apartamento (ej: 1-A, 1-B...)
  ...
  // Guardar o Actualizar Propietario en Supabase
  ...
  const listaFiltrada = propietarios.filter((item: any) =>
  ...
  // Nomenclatura exacta blindada para impresión
  const handlePrint = () => {
  ...
  // LÓGICA MATEMÁTICA DEL SEMÁFORO INTELIGENTE
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-4 font-serif">
        <form onSubmit={handleLogin} className="bg-neutral-900 border border-neutral-800 p-8 rounded-2xl shadow-2xl w-full max-w-sm">
          ...
          <input type="password" placeholder="Clave de administrador" className="w-full bg-black border border-neutral-800 rounded-lg p-4 text-center text-white font-bold tracking-widest outline-none focus:border-neutral-500 mb-6" value={password} onChange={e => setPassword(e.target.value)} />
  ...
  return (
    <div className="min-h-screen bg-black text-white font-serif antialiased pb-12 print:bg-white print:text-black print:p-0">
      ...
      {/* ENCABEZADO DE CONTROL CON PESTAÑAS */}
      <header className="no-print bg-neutral-900 border-b border-neutral-800 py-6 px-4 shadow-xl sticky top-0 z-50 backdrop-blur-md bg-opacity-95">
        <div className="max-w-4xl mx-auto flex flex-col items-center">
          <h1 className="text-2xl md:text-3xl font-bold text-white uppercase tracking-widest text-center">Torre D-10</h1>
          <p className="text-xs text-neutral-400 uppercase tracking-widest mt-1">Portal de Gestión de Residencia</p>
          
          <div className="flex gap-4 mt-6 bg-black p-1 rounded-xl border border-neutral-800 w-full max-w-md">
      ...
      <main className="max-w-4xl mx-auto px-4 pt-6">
        ...
        {/* PESTAÑA 1: CARTA DE RESIDENCIA (GENERADOR ORIGINAL) */}
        {activeTab === 'CARTA' && (
        ...
        {/* PESTAÑA 2: ACTUALIZAR DATA (EL SEMÁFORO INTELIGENTE REQUERIDO) */}
        {activeTab === 'ACTUALIZAR' && (
          <div className="no-print space-y-4 animate-fadeIn">
            <h2 className="text-xl font-bold uppercase tracking-wider text-neutral-300 mb-4">Censo del Edificio (Semáforo de Datos)</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {propietarios.map((item: any) => {
                ...
                return (
                  <div key={item.id} className={`p-5 rounded-xl border transition-all flex flex-col justify-between shadow-lg ${estilo.tarjeta}`}>
                    ...
                    <button onClick={() => setEditingProp(item)} className="w-full bg-neutral-900 hover:bg-neutral-800 border border-neutral-800 text-white font-bold py-2 rounded-lg text-xs uppercase tracking-wider transition-colors">Actualizar Datos</button>
                  </div>
                );
              })}
            </div>
        ...
        {/* MODAL / FORMULARIO POP-UP DE EDICIÓN FLOTANTE */}
        {editingProp && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 no-print animate-fadeIn">
            <form onSubmit={handleSavePropietario} className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 w-full max-w-md shadow-2xl space-y-4">
              <h3 className="text-lg font-bold uppercase tracking-wider border-b border-neutral-800 pb-2 text-white">Modificar Apartamento {editingProp.apartamento}</h3>
              ...
              <div>
                ...
                <input type="text" value={editingProp.propietario || ''} onChange={e => setEditingProp({...editingProp, propietario: e.target.value})} className="w-full bg-black border border-neutral-800 p-3 rounded-lg text-sm text-white" placeholder="Ej: SINDY CHACÓN" />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-widest text-neutral-400 mb-1">Cédula (Solo Número)</label>
                  <input type="text" value={editingProp.cedula || ''} onChange={e => setEditingProp({...editingProp, cedula: e.target.value})} className="w-full bg-black border border-neutral-800 p-3 rounded-lg text-sm text-white" placeholder="Ej: 17693292" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-widest text-neutral-400 mb-1">Piso</label>
                  <input type="text" value={editingProp.piso || ''} onChange={e => setEditingProp({...editingProp, piso: e.target.value})} className="w-full bg-black border border-neutral-800 p-3 rounded-lg text-sm text-white" placeholder="Ej: 1" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={editingProp.inicio_mes || ''} onChange={e => setEditingProp({...editingProp, inicio_mes: e.target.value})} className="w-full bg-black border border-neutral-800 p-3 rounded-lg text-sm text-white" placeholder="Ej: enero" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-widest text-neutral-400 mb-1">Año de Ingreso</label>
                  <input type="text" value={editingProp.inicio_ano || ''} onChange={e => setEditingProp({...editingProp, inicio_ano: e.target.value})} className="w-full bg-black border border-neutral-800 p-3 rounded-lg text-sm text-white" placeholder="Ej: 2018" />
                </div>
              </div>

              <div className="flex gap-3 pt-4 border-t border-neutral-800">
                <button type="button" onClick={() => setEditingProp(null)} className="flex-1 bg-neutral-800 hover:bg-neutral-700 text-white font-bold py-2.5 rounded-lg text-xs uppercase tracking-wider transition-colors">Cancelar</button>
                <button type="submit" className="flex-1 bg-white hover:bg-neutral-200 text-black font-bold py-2.5 rounded-lg text-xs uppercase tracking-wider transition-colors shadow-lg">Guardar Cambios</button>
              </div>
            </form>
          </div>
        )}
      ...
      {/* DOCUMENTO OFICIAL PARA LA IMPRESORA (RESTAURADO E INALTERADO) */}
          ...
          <>
            <div>
              ...
              <p className="mb-5 leading-[1.7] font-normal">
                Quienes suscriben, en representation del <strong className="font-bold">COMITÉ MULTIFAMILIAR DE GESTIÓN (C.M.G.) DE LA TORRE D-10</strong>, 
